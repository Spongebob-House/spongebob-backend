...
@PostMapping("/inter/{userId}/{dong}")
@Transactional
private ResponseEntity<String> addinter(@PathVariable("dong") @ApiParam(value = "동코드.", required = true) String dong, @PathVariable("userId") @ApiParam(value = "유저아이디.", required = true) String userId) throws SQLException{
    Map<String, String> map = new HashMap<String, String>();
    map.put("userid", userId);
    map.put("dong", dong);
    logger.debug("addinter call parameter {}", map);
    if(mapService.addinter(map)) {
		return new ResponseEntity<String>(SUCCESS, HttpStatus.OK);
	}
